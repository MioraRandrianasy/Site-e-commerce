console.log('mandehaaaaaa');
fetch('../assets/data/produits.json')
    .then(response => response.json())
    .then(products => {
        const panierContainer = ""
        const container = document.querySelector('.paquet-produits')
...
const addProduct = (product, container)=>{
    container.innerHTML += `
    <div class="produit" data-id = "${product.id}">
                        <div class="photo">
                        ...
                        <div class="details_produit">
                            ...
                            <div class="descrit-produit">
                                <p>Le doré et le terra cotta, le combo parfait ! Modèle à plateforme de 4 cm pour un maximum de confort.  Si vous êtes entre 2 pointures, optez pour...</p>
                            </div>
                        </div>
                    </div>
    ` 
